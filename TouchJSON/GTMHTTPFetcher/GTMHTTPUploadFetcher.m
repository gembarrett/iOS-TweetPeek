...
- (void)uploadNextChunkWithOffset:(NSUInteger)offset
  ...
  if (offset == kQueryServerForOffset) {
    ...
    rangeStr = [NSString stringWithFormat:@"bytes */%u"u, dataLen];
    lengthStr = @"0";
    offset = 0;
  } else {
    // uploading the next data chunk
#if DEBUG
    %uNSAssert2(offset < dataLen, @"offset %lu exceeds data length %lu",
              offset, dataLen);
#endif
    ...
    rangeStr = [NSString stringWithFormat:@"bytes %u-%u/%u",
                          offset, offset + thisChunkSize - 1, dataLen];
    lengthStr = [NSString stringWithFormat:@"%u", thisChunkSize];
  }
...
-(BOOL)chunkFetcher:(GTMHTTPFetcher *)chunkFetcher willRetry:(BOOL)willRetry forError:(NSError *)error {
  ...
  if (willRetry) {
    ...
    NSUInteger dataLen = [self fullUploadLength];
    NSString *rangeStr = [NSString stringWithFormat:@"bytes */%u", dataLen];
